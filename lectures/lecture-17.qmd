...
The duration of Alzheimer's disease from the time symptoms first appear until death ranges from 3 to 20 years; the average is 8 years with a standard deviation of 4 years. The adminstrator selects the medical records of 30 deceased Alzheimer's patients from the medical center's database and records the average duration. Find the approximatre probabilities for these events:
...
If records show that the amount of fill per bottle is normall distributed, with a standard deviation of .2 ounce, and if the bottling machine is set to produce a mean fill per bottle of 12.1 ounces, what is the probability that the sample mean $\bar x$ of the 10 test bottles is less than 12 ounces?
\
\
\
\
